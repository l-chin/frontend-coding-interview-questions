...
// since we are using the stack, the time complexity is O(n), because we are iterating through the string once
// since we are using the stack, the space complexity is O(n), because we are creating a new array
// this is a better solution because it handles multiple types of brackets

const isBalancedBetter2 = (str) => {
  let count = 0
  let arr = str.split('')
  for (let i = 0; i < arr.length; i++) {
...
test(t => t.is(isBalancedBetter2('{[()]}'), true))
test(t => t.is(isBalancedBetter2('{[(])}'), false))
test(t => t.is(isBalancedBetter2('([{}])'), true))
test(t => t.is(isBalancedBetter2('[{()}]'), true))
